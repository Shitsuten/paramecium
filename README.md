...
原始文本进来后，用 DS V3（便宜的小模型）拆分和提取成简短的记忆条目——使用第一人称且具有主观情感色彩，然后进行向量化。
...
默认只拉取固定条目的上下文原文；每过几轮对话会有小模型进行总结。顺带一提，参考其他老师的省钱思路：图片会用小模型进行转述后传递给下一轮 API 对话调用（经核实目前只做了图片转述。正在严肃鞭策 CC 把工具调用结果的转述也做了）。
