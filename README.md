...
一个人、一台 VPS、一套完整的 AI 对话系统。不需要 Kubernetes，不需要 Redis，不需要任何你在大厂架构图里见过的东西。

Paramecium 是一个轻量级 AI 网关 + 记忆系统，设计目标是：**让一个非 CS 专业的人也能搭建自己的、有记忆的、省钱的 AI 对话后端**。
...
4. **热度衰减** — 不常被召回的记忆自动降温，注入时优先推热的
5. **可视化后台** — 带一个 admin 面板，能看记忆列表、搜索、力导向图谱
