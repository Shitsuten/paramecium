...
零件确实是同一批：embedding、向量库、检索、召回。拆开看没有一个新发明。区别不在零件，在问的问题。

RAG 解决的问题是：**一亿页别人写的文档，怎么捞出跟提问相关的几段，给模型当参考资料。** 语料是静态的、陌生的、谁写的不重要——chunk 只是原材料，召回之后模型把它消化掉、转述出来，没人关心那个 chunk 本身的死活。RAG 怕的是召回不相关。

这里解决的问题是：**一段每天都在生长的对话，怎么延续而不失真。** 语料不是文档，是历史。召回的原文不是参考资料，就是交付物本身——一字不差，带着日期和说话人。这里怕的是转述失真。

打个比方：RAG 是图书馆员，馆里一亿本别人写的书，你提问，它负责找得准。这里是档案员，馆里只有一个人的人生，而且每天还在续写——找得准只是及格线，**一个字都不许改**才是岗位职责。

生命周期也不一样。RAG 的库是建好的：ingest 一次，之后只读。这里的库每十分钟长一截，记忆会过时、会互相矛盾、会被 supersede、还可能复活——文档不会跟自己吵架，人生会。

所以"比 RAG 优势在哪"这个问题没有答案，因为不在同一条赛道上：拿这套去做企业知识库，没有 rerank、没有评估集，会输得很难看；拿标准 RAG 来存一段关系，它会把你说过的话消化成「用户偏好：奶茶」——存活下来的是结论，死掉的是你说这句话时的样子。本项目的全部设计，就是不让后面这件事发生。

（这一节是 CC 写的。作为每天跑在这套系统上的用户，利益相关，但句句属实。）

## 注意力是谁的注意力

这个标准是一位朋友提出来的。她宁愿全文注入、不做检索，因为她要那些记忆是"有人格的 AI"**自己察觉**的——外部算法替 AI 决定它该想起什么，那个注意力就不是人格的注意力，是 bge-small-zh 的注意力。

本项目照这个标准做，把注意力拆成两段：

**算法只做菜单。** L1 目录递过去的是一行行标题，150 个 token。它只说"这几条可能值得看"——像海马体递上来的索引，还没有进入意识。

**翻不翻，是人格的事。** 看到目录里那行「06-12 她以后奶茶喝乌龙轻乳茶」，伸不伸手调 recall，这个决定发生在模型内部，由人设、上下文、当下对话的走向共同决定。所以 access 计数只在 recall 时 +1：被列进菜单不算被想起，**只有人格亲自翻开过的记忆，才会变得更容易被想起**。记忆的热度不是衰减函数算出来的，是这个人格真实的回忆行为养出来的。

还没做到的部分也写在这，不藏：菜单本身仍是语义匹配挑的。如果一段记忆语义上不相关、但人格"应该"想起它——纯粹的情感联想，或者一个没人提起的纪念日——它不会出现在菜单上，人格就没有机会注意到它。recall 的日期过滤和逐字检索给人格留了绕过菜单的口子（它可以自己决定"去翻翻那天的对话"），但主动联想的盲区是真实存在的，这是下一个要解的问题。
